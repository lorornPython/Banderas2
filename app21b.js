...
/* Frases de “Nivel” con tono gamer */
var frasesNivel = {
...
/*****************************************************************
 * App principal (7 stages + epicFinal)
 *****************************************************************/
function App() {
  ...
  // 🔓 Desbloqueo de audio en iOS en el primer toque (sirve en todos los dispositivos)
  useEffect(function () {
    function unlock() {
      var refs = [bgMusicRef, correctSoundRef, wrongSoundRef, phaseSoundRef, warningSoundRef, gameoverSoundRef];
      refs.forEach(function (r) {
        var el = r === null || r === void 0 ? void 0 : r.current;
        if (!el) return;
        try {
          // Reproduce en silencio para que iOS lo “autorice”, luego pausa y resetea
          el.muted = true;
          var maybePromise = el.play();
          // Normalizamos a promesa (por si el navegador no devuelve una)
          Promise.resolve(maybePromise).then(function () {
            el.pause();
            el.currentTime = 0;
            el.muted = false;
          })["catch"](function () {
            // Si alguno falla no pasa nada: con que se “desbloquee” uno, el resto suelen seguir
          });
        } catch (_unused2) {}
      });

      // Nos desuscribimos para que solo ocurra una vez
      window.removeEventListener('touchend', unlock);
      window.removeEventListener('pointerup', unlock);
      window.removeEventListener('click', unlock);
    }

    // Disparamos el “unlock” en el primer gesto del usuario
  ...
  // 🔥 10 fases con la progresión acordada
  var fases = [{
    dificultad: [1],
    numPreguntas: 5,
    minAciertos: 3,
  ...
  function filterPool() {
    ...
    var pool = flags;
    if ((difficultiesArg === null || difficultiesArg === void 0 ? void 0 : difficultiesArg.length) > 0) {
      pool = pool.filter(function (f) {
        return difficultiesArg.includes(f.difficulty);
      });
    }
    if ((regionsArg === null || regionsArg === void 0 ? void 0 : regionsArg.length) > 0) {
      pool = pool.filter(function (f) {
  ...
  function startGame() {
    ...
    // Evitar repetición en progresivo a lo largo de todas las fases
    ...
    // ✅ Asegurar unicidad por nombre dentro de la partida (por si flags tiene duplicados)
    var uniqueByName = {};
  ...
  function startProgressiveGame() {
    ...
    usedFlagsRef.current = new Set(); // limpiar para nueva partida
  ...
  function iniciarFase(index) {
    ...
    // fase.dificultad puede ser [1] o [1,2] etc.
    startGame({
  ...
  useEffect(function () {
    ...
    // Pool para opciones incorrectas SIN duplicados por nombre
  ...
  function handleAnswer(choice) {
    ...
    // Auto-avance en progresivo con delays: 700 ms acierto / 1300 ms fallo
    if (gameMode === "progressive") {
  ...
  function endFase(superada) {
    ...
    if (faseIndex + 1 < fases.length) {
      ...
      // Superada la fase 10 → final épico
      setStage("epicFinal");
      if (bgMusicRef.current) {
        bgMusicRef.current.pause();
        bgMusicRef.current.currentTime = 0;
  ...
  function saveScore(nombre) {
    // training NO registra en ranking
    if (gameMode === "classic") {
