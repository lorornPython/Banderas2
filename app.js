...
/*****************************************************************
 * DATA:
 * Aquí va la lista completa de banderas (cópiala de tu archivo estable).
 *****************************************************************/
var flags = [
// ⚠️ Pega aquí toda la lista de banderas de tu archivo estable.
...
function App() {
  ...
  function filterPool(customDifficulty) {
    var pool = flags.filter(function (f) {
      return f.difficulty === Number(customDifficulty !== null && customDifficulty !== void 0 ? customDifficulty : difficulty);
    });
  ...
  function startGame(customDifficulty, customNumQuestions) {
    var diff = customDifficulty !== null && customDifficulty !== void 0 ? customDifficulty : difficulty;
    var nQuestions = customNumQuestions !== null && customNumQuestions !== void 0 ? customNumQuestions : numQuestions;
    var avoidSet = gameMode === "progressive" ? usedFlagsRef.current : new Set();
    var pool = filterPool(diff).filter(function (f) {
    ...
    var n = Math.min(nQuestions, pool.length);
  ...
  function iniciarFase(index) {
    ...
    // Llamamos a startGame con los valores de la fase
    startGame(fase.dificultad, fase.numPreguntas);
  ...
  }, "Ver Ranking"))), stage === "game" && (questions.length === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "text-center py-12 text-lg text-slate-600"
  }, "Cargando preguntas...") : function () {
    var current = questions[currentIndex];
