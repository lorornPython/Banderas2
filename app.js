 ...
 * DATA
 *****************************************************************/
// ⚠️ Pega aquí tu array completo desde tu archivo local:
var flags = /* <-- PÉGALO AQUÍ: const flags = [ ... ]; */[
// 🌍 EUROPA (44 países)
{
  name: "Albania",
  code: "al",
...
function App() {
  ...
  function filterPool() {
    ...
    var featureArg = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : feature;
    ...
    if (featureArg !== "Todos") {
      if (featureArg === "Con Azul") pool = pool.filter(function (f) {
        return f.hasBlue;
      });
      if (featureArg === "Con Rojo") pool = pool.filter(function (f) {
        return f.hasRed;
      });
      if (featureArg === "Con Blanco") pool = pool.filter(function (f) {
        return f.hasWhite;
      });
      if (featureArg === "Con Amarillo") pool = pool.filter(function (f) {
        return f.hasYellow;
      });
      if (featureArg === "Con Estrellas") pool = pool.filter(function (f) {
        return f.hasStars;
      });
      if (featureArg === "Con Rayas") pool = pool.filter(function (f) {
        return f.hasStripes;
      });
    }
    return pool;
  ...
  function startGame() {
    ...
    var pool = filterPool(effDifficulty, region, feature).filter(function (f) {
  ...
  }, "Nota: la lista actual incluye ", /*#__PURE__*/React.createElement("strong", null, flags.length), " pa\xEDses. Puedes ajustar filtros si quieres menos/otros.")), stage === "game" && function () {
    var current = questions[currentIndex];
    // Fallback seguro si aún no hay pregunta lista
    if (!current) {
      return /*#__PURE__*/React.createElement("div", {
